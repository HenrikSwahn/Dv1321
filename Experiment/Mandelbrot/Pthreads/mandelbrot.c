...
void mandelbrot(int x, int y) {
	...
	for (i = y; i < height; i++) {
		for (j = x; j < width; j++) {
			float b = xmin + j * (xmax - xmin) / 1024;
			float a = ymin + i * (ymax - ymin) / 1024;
			float sx = 0.0f;
			float sy = 0.0f;
			int ii = 0;
			while (sx + sy <= 64.0f) {
			...
			if (ii == 200)	{
				pixmap[j+i*(int)1024] = 0;
			}
			else {
				int c = (int)((ii / 32.0f) * 256.0f);
				pixmap[j + i *(int)1024] = pal[c%256]; 
			}
		}
	}
}
...
int main(int a, char *args[]) {
	...
	for(; i < MAX_THREADS*2; i++) {
		...
		*arg = i;
		pthread_create(&tid[threadCounter++], 0, fractalWorker, arg);
